...
public class StaffDAO extends DBContext {
    ...
    public List<Staff> getStaffbyStatus() {
        CompanyDAO sd = new CompanyDAO();
        RoleDAO rd = new RoleDAO();
        String sql = "select distinct Staff.* from Request right join Staff on Request.sId = Staff.Id where Request.Status = '1'";
        List<Staff> list = new ArrayList<>();
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
    ...
    public List<Staff> getAllAdmin() {
        CompanyDAO sd = new CompanyDAO();
        RoleDAO rd = new RoleDAO();
        String sql = "select * from Staff where roleid= 0 and status = 1";
        List<Staff> list = new ArrayList<>();
        try {
            PreparedStatement st = connection.prepareStatement(sql);
...
//        System.out.println("" + dao.getWorkingStaff("5"));
        System.out.println(dao.checkDuplicatateUsername("quangdpcl"));
    }
